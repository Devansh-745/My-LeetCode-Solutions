class Solution {
    public:
    // dp[i][j][move][dir] stores the maximum length of a V-shaped segment starting at cell (i,j)
    // with a given number of allowed turns (move) and current diagonal direction (dir).
    int dp[500][500][2][4];
    int r, c; // dimensions of the grid

    // fxn recursively computes the length of the valid segment from position (i, j)
    // Parameters:
    //   grid - the input grid
    ...
    //   prev - the value of the previous cell in the sequence (to enforce the sequence rule)
    int fxn(vector<vector<int>>& grid, int i, int j, int move, int dir, int prev) {
        // Out-of-bound check OR if the cell equals the previous value (violating the sequence)
        // OR if the cell equals 1 (which can only be the starting point)
        if (i < 0 or j < 0 or i >= r or j >= c or grid[i][j] == prev or grid[i][j] == 1) {
            return 0;
        }
        // If the previous cell was 1, then a cell with 0 is not allowed in the sequence (since sequence should be 1,2,0,2,0,...)
        ...
        int op1 = 0, op2 = 0; // options: turning or continuing in the same direction
    ...
    // Main function to calculate the length of the longest V-shaped diagonal segment.
    int lenOfVDiagonal(vector<vector<int>>& grid) {
        ...
        for (int i = 0; i < r; i++) {
            for (int j = 0; j < c; j++) {
                ...
                if (grid[i][j] == 1) {
                    // Try starting in all four diagonal directions with 1 turn allowed.
                    // The starting cell (value 1) is counted separately, hence the +1.
                    ans = max(ans, 1 + fxn(grid, i - 1, j - 1, 1, 0, 1)); // Direction 0: up-left turn option.
                    ans = max(ans, 1 + fxn(grid, i - 1, j + 1, 1, 1, 1)); // Direction 1: up-right turn option.
                    ans = max(ans, 1 + fxn(grid, i + 1, j + 1, 1, 2, 1)); // Direction 2: down-right turn option.
                    ans = max(ans, 1 + fxn(grid, i + 1, j - 1, 1, 3, 1)); // Direction 3: down-left turn option.
                }
            }
        }
